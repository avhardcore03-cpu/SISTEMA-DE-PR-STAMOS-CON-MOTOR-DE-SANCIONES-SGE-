...
const Inventario = () => {
  ...
  const obtenerInventario = async () => {
    try {
      const res = await fetch("http://localhost:3001/inventario");
  ...
  const agregarProducto = async () => {
    if (!nombre || !cantidad) return alert("Completa los campos");
    try {
      const res = await fetch("http://localhost:3001/inventario", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre, cantidad: Number(cantidad) }),
      });
      if (res.ok) {
  ...
  const eliminarProducto = async (id) => {
    try {
      const res = await fetch(`http://localhost:3001/inventario/${id}`, { method: "DELETE" });
