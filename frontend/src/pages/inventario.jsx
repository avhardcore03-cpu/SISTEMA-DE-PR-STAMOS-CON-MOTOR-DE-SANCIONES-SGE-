...
const Inventario = () => {
  ...
  const eliminarEquipo = async (id) => {
    if (!window.confirm("¿Estás seguro de eliminar este equipo del sistema?"))
      return;
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Gestiona el stock de equipos tecnológicos de la institución.
          </p>
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-8 transition-all hover:shadow-md">
        ...
        >
          ...
          <div className="flex gap-2">
            <button
              className={`flex-1 p-3 rounded-xl font-bold text-white transition-all active:scale-95 shadow-lg ${editandoId ? "bg-orange-500 shadow-orange-100" : "bg-emerald-600 shadow-emerald-100"}`}
            >
            ...
            {editandoId && (
              <button
                ...
                className="p-3 bg-slate-200 text-slate-600 rounded-xl font-bold"
              >
                Cancelar
              </button>
            )}
              ...
              >
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <div
                      className={`h-2 w-2 rounded-full ${eq.cantidad > 0 ? "bg-emerald-500" : "bg-red-500"}`}
                    ></div>
                ...
                <td className="p-4">
                  <div className="flex justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => prepararEdicion(eq)}
                      className="text-blue-500 hover:text-blue-700 text-sm font-bold flex items-center gap-1"
                    >
                      <span>Editar</span>
                    </button>
                    <button
