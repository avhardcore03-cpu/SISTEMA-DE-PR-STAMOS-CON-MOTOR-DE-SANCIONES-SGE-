...
export const crearReserva = async (req, res) => {
  ...
  try {
    const equipo = await pool.query('SELECT estado, stock_total FROM equipos WHERE id = $1', [id_equipo]);
    ...
    await db.query("UPDATE equipos SET estado = 'RESERVADO' WHERE id = $1", [id_equipo]);
...
export const aprobarReserva = async (req, res) => {
  ...
  try {
    ...
    const reserva = await pool.query('SELECT id_equipo FROM prestamos WHERE id = $1', [id_reserva]);
    if (reserva.rows.length > 0) {
      await pool.query("UPDATE equipos SET estado = 'EN_PRESTAMO' WHERE id = $1", [reserva.rows[0].id_equipo]);
    }
...
export const cancelarReserva = async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE prestamos SET estado = 'CANCELADO' WHERE id = $1", [id_reserva]);
    await pool.query("UPDATE equipos SET estado = 'DISPONIBLE' WHERE id = $1", [id_equipo]);
