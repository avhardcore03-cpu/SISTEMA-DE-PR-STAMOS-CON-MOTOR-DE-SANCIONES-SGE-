...
const generarToken = (usuario) => {
  ...
  return jwt.sign(payload, config.jwtSecret || "secreto_super_seguro_123", {
    expiresIn: "1h",
  });
};
...
export const obtenerPerfil = (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ exito: false, message: "Error al obtener el perfil" });
  }
};
...
export const validarToken = (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ exito: false, message: "Error al validar el token" });
  }
};
